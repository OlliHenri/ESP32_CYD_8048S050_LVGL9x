...
static void touch_event(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED)
    {
        ...
        lv_obj_set_style_text_color(obj, lv_color_make(n, n, n), 0);
    }
}
...
void lcd_init(void *)
{
    ...
    /* // block everything else by taking a semaphore
    if( xSemaphore != NULL )
    {
    ...
    ESP_LOGI(TAG, "Printed the label");
    ...
    while (1)
    {
        xSemaphoreTake( xSemaphore, ( TickType_t ) 10 );    // take semaphore
...
{   static int n = 0;
	...
	{
        //xSemaphoreTake( xSemaphore, ( TickType_t ) 10 );    // take semaphore
        ESP_LOGI(TAG, "here is do_it");
        ++n;
        printf("do_it counter is= %d", n);
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "Starting lvgl app_main");
   ...
   if( xSemaphore != NULL )
   {
       ...
       ESP_LOGI(TAG, "The semaphore was created successfully");
   }

    // start LVGL related calls on core #1.
    xTaskCreatePinnedToCore(lcd_init, "lcd_init", 8192, NULL, 1, NULL, 1);
    xTaskCreatePinnedToCore(do_it, "do_it", 8192, NULL, 2, NULL, 1);
